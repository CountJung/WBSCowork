...
> 현재 코드의 탐색 지도. 진행 이력과 장기 backlog는 기존 `docs/PROJECT_MAP.md`를 함께 본다.
...
| `scripts/` | 실제 CLI 하네스 | `run-next.ts`, `check-db.ts`, 문서 변환 스크립트 |
| `docs/` | 기존 계획·사용자 문서·FSD/digest 설계 | `MasterPlan.md`, `PROJECT_MAP.md`, `FSD_MIGRATION_PLAN.md` |
| `.github/` | Copilot 규칙과 저장소 skills | instructions, document skills |
| `tasks/` | 작업 인계 템플릿 | `TASK_TEMPLATE.md` |

현재 `src/`는 없다. FSD는 목표 구조이지 완료 구조가 아니다.
...
- comment/attachment project 조회는 부모 submission visibility와 동일한 filter를 받지 않는다.
...
| FSD 이동 | public API, server/client boundary, alias `@/*`, legacy re-export |
...
3. 루트 운영 문서(`AGENTS.md`, 이 문서, `ARCHITECTURE.md`, `HARNESS_MAP.md`)
4. `docs/`의 계획/이력 문서
